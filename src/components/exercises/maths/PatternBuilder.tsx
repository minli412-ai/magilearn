...
export default function PatternBuilder({ question, onAnswer }: Props) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {Array.isArray(question.extraData?.sequence) && (
        <div className="flex gap-2 justify-center mb-4 flex-wrap">
          {(question.extraData!.sequence as (number | string | null)[]).map((val, i) => (
            <div key={i} className={`w-14 h-14 rounded-xl flex items-center justify-center text-xl font-bold font-literacy ${
              val === null ? 'bg-[var(--color-primary)]/30 border-2 border-dashed border-[var(--color-primary)]' : 'bg-white/10'
            }`}>
              {val === null ? '?' : String(val)}
            </div>
          ))}
